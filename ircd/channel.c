 ...
 *
 * $Id: channel.c,v 1.1.1.1 2006/12/19 12:55:00 zipbreake Exp $
 */
#include "config.h"
...
void set_owner(struct JoinBuf *jbuf, int type, struct Channel *chan);
...
{
  struct SLink* tmp;
  char          nu_virtualhost[NUH_BUFSIZE];
  ...
  if (cli_flags(cptr) & FLAGS_HIDDENHOST)
    sr = make_nick_user_host(nu_virtualhost, cli_name(cptr),
			     (cli_user(cptr))->username,
			     cli_user(cptr)->virtualhost);
...
void remove_user_from_channel(struct Client* cptr, struct Channel* chptr)
{
  ...
  /* Si el usuario que abandona el canal es el fundador, lo cambiamos en chptr */
  if (chptr->founder == cptr)
    chptr->founder = NULL;
...
int is_chan_halfop(struct Client *cptr, struct Channel *chptr)
{
	struct Membership *member;
	assert(chptr);
	if ((member = find_member_link(chptr, cptr)))
		return (!IsZombie(member) && IsChanHalfop(member));
...
int member_can_send_to_channel(struct Membership* member)
{
  ...
  if ((member->channel->rhmode.mode & RHMODE_REGMOD) && !IsRegnick(member->user))
    return 0;
...
int client_can_send_to_channel(struct Client *cptr, struct Channel *chptr)
{
  ...
  if (!member) {
    if ((chptr->mode.mode & (MODE_NOPRIVMSGS|MODE_MODERATED)) ||
		((chptr->rhmode.mode & RHMODE_REGMOD) && !IsRegnick(cptr)) ||
...
*/  if (chptr->mode.limit) {
  ...
  if (chptr->rhmode.mode & RHMODE_NOCOLOR)
	  *mbuf++ = 'c';
  if (chptr->rhmode.mode & RHMODE_RGSTRONLY)
	  *mbuf++ = 'R';
  /* El modo +r y +j es gestionado por base de datos y no debe enviarse entre
     servidores */
  if (!IsServer(cptr))
  {
    if (chptr->rhmode.mode & RHMODE_REGISTERED)
	  *mbuf++ = 'r';
    if (chptr->rhmode.mode & RHMODE_HASJOINP)
	  *mbuf++ = 'j';
  }
  if (chptr->rhmode.mode & RHMODE_AUTOOP)
	  *mbuf++ = 'A';
  if (chptr->rhmode.mode & RHMODE_ONLYIRCOP)
	  *mbuf++ = 'O';
  if (chptr->rhmode.mode & RHMODE_CHARLAS)
	  *mbuf++ = 'C';
  if (chptr->rhmode.mode & RHMODE_SUSPEND)
	  *mbuf++ = 'S';
  if (chptr->rhmode.mode & RHMODE_REGMOD)
	  *mbuf++ = 'M';
...
void send_channel_modes(struct Client *cptr, struct Channel *chptr)
{
  static unsigned int current_flags[16] =
      { 0,
	  CHFL_OWNER | CHFL_CHANOP | CHFL_HALFOP | CHFL_VOICE,
	  CHFL_OWNER | CHFL_CHANOP | CHFL_HALFOP,
	  CHFL_OWNER | CHFL_CHANOP | CHFL_VOICE,
	  CHFL_OWNER | CHFL_CHANOP,
	  CHFL_OWNER | CHFL_HALFOP | CHFL_VOICE,
	  CHFL_OWNER | CHFL_HALFOP,
  ...
  *modebuf = *parabuf = '\0';
  ...
  for (first = 1; full; first = 0)      /* Loop for multiple messages */
  {
    ...
    mb = msgq_make(&me, "%C %s %H %Tu", &me, usa_tokens(cptr) ? TOK_BURST : MSG_BURST,
	...
	if (msgq_bufleft(mb) < NUMNICKLEN + 6)
          /* The 6 is a possible ",:ovhq" */
        {
	 ...
	 * 16 Junio 2003
	 * RyDeN
	 * Soporte para el TBURST añadido
	 *
	 */
	 if (*(chptr->topic) != '\0') {
	    mb = msgq_make(&me, "%C %s %H %Tu %s :%s", &me,
			usa_tokens(cptr) ? TOK_TBURST : MSG_TBURST,
			chptr, chptr->topic_time, chptr->topic_nick,
			chptr->topic);
...
int can_join(struct Client *sptr, struct Channel *chptr, char *key)
{
   ...
   */
	...
	if ((reg = db2_buscar_subclave(BDD_CHANNELS, chptr->chname, BDD_CHANNELS_FOUNDER)) ||
		(reg = db2_buscar_subclave(BDD_CHANNELS, chptr->chname, BDD_CHANNELS_SUCESSOR)))
	{
		if (!ircd_strcmp(cli_name(sptr), reg->valor))
			return 0;
	}
...
/*  if (IsLocalChannel(chptr->chname) && HasPriv(sptr, PRIV_WALK_LCHAN) &&
  ...
  if ((cli_flags(sptr) & FLAGS_CHSERV) || es_representante(sptr) || IsPreoper(sptr)) {
	  overrideJoin = MAGIC_OPER_OVERRIDE;
  }
  ...
  if ((chptr->rhmode.mode & RHMODE_RGSTRONLY) && !IsRegnick(sptr))
...
inline void channel_set_persistent_modes(struct Channel *chptr)
{
    /* Ajustamos los modos permanentes */
	if (db_buscar_registro(BDD_JOINS, chptr->chname))
		chptr->rhmode.mode |= RHMODE_HASJOINP;
	if (db2_buscar_registro(BDD_CHANNELS, chptr->chname))
...
struct Channel *get_channel(struct Client *cptr, char *chname, ChannelGetType flag)
{
  ...
  if (flag == CGT_CREATE)
  {
    ...
    hAddChannel(chptr);
    channel_set_persistent_modes(chptr);
  }
  return chptr;
}
...
/*	  RHMODE_HALFOP,	'h',
  ...
  if (mbuf->mb_add == 0 && mbuf->mb_rem == 0 && mbuf->mb_count == 0 && mbuf->mb_rhadd == 0 && mbuf->mb_rhrem == 0)
    return 0;

  for (i=0;MB_TYPE(mbuf, i) != 0;i++);
	if (mbuf->mb_count > i)
		return;
  ...
  for (flag_p = rhflags; flag_p[0]; flag_p += 2) {
	  if (*flag_p & mbuf->mb_rhadd)
		  addbuf[addbuf_i++] = flag_p[1];
	  else if (*flag_p & mbuf->mb_rhrem)
		  rembuf[rembuf_i++] = flag_p[1];
  }
  ...
  for (i = 0; i < mbuf->mb_count; i++) {
    ...
    if (MB_TYPE(mbuf, i) & (MODE_CHANOP | MODE_VOICE | CHFL_OWNER | CHFL_HALFOP)) {
      tmp = strlen(cli_name(MB_CLIENT(mbuf, i)));

      if ((totalbuflen - IRCD_MAX(5, tmp)) <= 0) /* don't overflow buffer */
	MB_TYPE(mbuf, i) |= MODE_SAVE; /* save for later */
      else {
		  unsigned int cmpl = MB_TYPE(mbuf, i);
		  char result;
		  if (cmpl & MODE_CHANOP)
			  result = 'o';
		  else if (cmpl & MODE_VOICE)
			  result = 'v';
		  else if (cmpl & CHFL_HALFOP)
			  result = 'h';
		  else
			  result = 'q';
		  bufptr[(*bufptr_i)++] = result;
  ...
  if ((mbuf->mb_dest & (MODEBUF_DEST_CHANNEL | MODEBUF_DEST_HACK2 |
    ...
    if (mbuf->mb_dest & MODEBUF_DEST_HACK4 && IsServer(mbuf->mb_source))
    ...
    if (mbuf->mb_dest & MODEBUF_DEST_LOG)
      log_write(LS_OPERMODE, L_INFO, LOG_NOSNOTICE,
		...
		if (mbuf->mb_dest & MODEBUF_DEST_CHANNEL) {
			if (mbuf->mb_dest & MODEBUF_BOTMODE)
			{
				struct db_reg *reg;
				char *botname;

				if ((reg = db_buscar_registro(BDD_BOTS, mbuf->botmode_from)) && (reg->valor))
					botname = reg->valor;
				else
					botname = cli_name(&me);

				sendcmdto_channel_butserv_butone_botmode(botname, CMD_MODE,
					mbuf->mb_channel, NULL,
  ...
  if (!IsLocalChannel(mbuf->mb_channel->chname) && (mbuf->mb_dest & MODEBUF_DEST_SERVER)) {
    ...
    if (mbuf->mb_dest & MODEBUF_DEST_OPMODE) {
       ...
       */
		/* Soporte para el BOTMODE */
		if (mbuf->mb_dest & MODEBUF_BOTMODE)
		{
			sendcmdto_serv_butone(mbuf->mb_source, CMD_BMODE, mbuf->mb_connect,
				      "%s %H %s%s%s%s%s%s %Tu", mbuf->botmode_from, mbuf->mb_channel,
				      rembuf_i ? "-" : "", rembuf, addbuf_i ? "+" : "",
					  addbuf, remstr, addstr,
				      mbuf->mb_channel->creationtime);
		}
...
void
modebuf_rhmode(struct ModeBuf *mbuf, unsigned int mode)
{
  assert(0 != mbuf);
  assert(0 != (mode & (MODE_ADD | MODE_DEL)));
...
modebuf_mode_uint(struct ModeBuf *mbuf, unsigned int mode, unsigned int uint)
{
  ...
  if (++mbuf->mb_count >=
      (MAXMODEPARAMS -
      (mbuf->mb_dest & MODEBUF_DEST_DEOP ? 1 : 0)))
...
mode_parse_limit(struct ParseState *state, int *flag_p)
{
  ...
  /* El half op no puede cambiar el limite */
  if (state->flags & MODE_PARSE_HALFOP) {
...
mode_parse_key(struct ParseState *state, int *flag_p)
{
  ...
  /* El half op no puede cambiar el key */
  if (state->flags & MODE_PARSE_HALFOP) {
	  send_reply(state->sptr, ERR_CHANOPRIVSNEEDED, state->chptr->chname);
	  return;
  }
...
mode_parse_client(struct ParseState *state, int *flag_p)
{
  ...
  state->parc--;
  state->max_args--;
  if (state->flags & MODE_PARSE_NOTADD) {
	  state->flags &= ~MODE_PARSE_NOTADD;
	  return;
  }

  /* El half op no puede cambiar el modo de un usuario a no ser que sea el +v */
  if ((state->flags & MODE_PARSE_HALFOP) && (flag_p[0] != CHFL_VOICE)) {
	  send_notoper(state);
	  return;
  }
...
*/
		if (! ((state->cli_change[i].client == state->cptr) || IsServer(state->cptr)) )
	    if (MyUser(state->sptr) && state->flags & MODE_PARSE_SET) {
	...
	member->status |= (state->cli_change[i].flag &
			   (MODE_CHANOP | MODE_VOICE | CHFL_OWNER | CHFL_HALFOP));
...
mode_parse_mode(struct ParseState *state, int *flag_p)
{
  ...
  if (state->dir == MODE_ADD) {
	  if (state->chgtype == 0) {
		...
		if (flag_p[0] & MODE_SECRET) {
		  ...
		  state->rhadd |= flag_p[0];
		  state->rhdel &= ~flag_p[0];
	  }
  } else {
	  if (state->chgtype == 0) {
		state->add &= ~flag_p[0];
  ...
  assert(0 == ((state->chgtype == 0) ? (state->add & state->del) : (state->rhadd & state->rhdel)));
...
//    MODE_REGONLY,	'r',
  ...
  state.numbans = 0;
  /* Tipo de cambio de modos.
	0 -> Modos nativos del ircu
	1 -> Modos propios de redhispana
  */
  state.chgtype = 0;
  ...
  while (*modestr) {
    ...
    for (; *modestr; modestr++) {
      for (flag_p = chan_flags; flag_p[0]; flag_p += 2) /* look up flag */
		  ...
		  if (!cerrojo) {
		for (flag_p = chan_rhflags; flag_p[0]; flag_p += 2)
			if (flag_p[1] == *modestr) {
				state.chgtype = 1;
				break;
			}
...
/*		  if ((chptr->mode.limit && state.dir == MODE_DEL) ||
			  (chptr->mode.limit == 0 && state.dir == MODE_ADD))
*/
            if ((chptr->mode.limit == 0) && (state.dir == MODE_DEL))
              break;
			mode_parse_limit(&state, flag_p);
	break;

      case 'k': /* deal with keys */
		  if ((*chptr->mode.key && state.dir == MODE_DEL) ||
			  (!*chptr->mode.key && state.dir == MODE_ADD))
	...
	break;
	  case 'q':
		  if (es_representante(cptr) && state.dir == MODE_DEL)
		  {
			  mode_parse_client(&state, flag_p);
			  break;
		  }
		  if (!IsServer(cptr) || (state.dir == MODE_ADD && !(mbuf->mb_dest & MODEBUF_BOTMODE)))
			state.flags |= MODE_PARSE_NOTADD;
		  mode_parse_client(&state, flag_p);
		  break;
	  case 'O':
		  if (!IsAnOper(sptr) && !IsServer(sptr) && !IsBot(sptr) && !IsUserbot(sptr))
			  break;
	  ...
	  case 'C':
	  case 'A':
			if (!IsBot(sptr) && !es_representante(sptr) && !is_chan_owner(sptr, chptr)) {
				send_reply(sptr, ERR_NOTOWNER, chptr->chname, *modestr);
				break;
			}
			mode_parse_mode(&state, flag_p);
			break;
   ...
   */
  if (!state.mbuf || (!(state.flags & MODE_PARSE_HALFOP) &&
	  state.flags & (MODE_PARSE_NOTOPER | MODE_PARSE_NOTMEMBER)))
	  if (!state.cli_change[0].client)
		return state.args_used; /* tell our parent how many args we gobbled */

  t_mode = state.chptr->mode.mode;
  t_rhmode = state.chptr->rhmode.mode;

	/* MODOS NATIVOS DE REDHISPANA */
  ...
  /* MODOS DE REDHISPANA */
  if (state.rhdel & t_rhmode) { /* delete any modes to be deleted... */
    modebuf_rhmode(state.mbuf, MODE_DEL | (state.rhdel & t_rhmode));

    t_rhmode &= ~state.rhdel;
  }
  if (state.rhadd & ~t_rhmode) { /* add any modes to be added... */
    modebuf_rhmode(state.mbuf, MODE_ADD | (state.rhadd & ~t_rhmode));

    t_rhmode |= state.rhadd;
  }
  /**/
  ...
  if (state.flags & MODE_PARSE_SET) { /* set the channel modes */
	...
	state.chptr->rhmode.mode = t_rhmode;
  }
...
joinbuf_join(struct JoinBuf *jbuf, struct Channel *chan, unsigned int flags)
{
  ...
  if (jbuf->jb_type == JOINBUF_TYPE_PART ||
	...
	if (jbuf->jb_type == JOINBUF_TYPE_JOIN)
		set_owner(jbuf, JOINBUF_TYPE_JOIN, chan);
...
void set_owner(struct JoinBuf *jbuf, int type, struct Channel *chan)
{
	...
	if (type == JOINBUF_TYPE_JOIN)
	{
	    if (chan->founder != NULL)
	    {
	         /* El canal ya tiene un fundador */
	         return;
	    }
	    
	   	if ((cli_rhflags(jbuf->jb_source) & RHFLAGS_REGNICK) && 
			(reg = db2_buscar_subclave(BDD_CHANNELS, chan->chname, BDD_CHANNELS_FOUNDER)) &&
		  (reg->valor) && !ircd_strcmp(reg->valor, cli_name(jbuf->jb_source)))
		{
			struct Membership *m;

			m = find_member_link(chan, jbuf->jb_source);
			assert(0 != m);
			m->status |= CHFL_OWNER;

			reg2 = db_buscar_registro(BDD_BOTS, BDD_BOTS_CHANSERV);
			sendcmdto_channel_butservs_butone_botmode(reg2 ? reg2->valor : cli_name(&me),
				CMD_MODE, chan, NULL, "%s +q %C", chan->chname, jbuf->jb_source);

			sendcmdto_serv_butone(&me, CMD_BMODE, NULL, BDD_BOTS_CHANSERV " %s +q %C",
				chan->chname, jbuf->jb_source);
			chan->founder = jbuf->jb_source;
		}
	} else if (type == JOINBUF_TYPE_CREATE)
	{
		for (i = 0; i < jbuf->jb_count; i++)
			set_owner(jbuf, JOINBUF_TYPE_JOIN, jbuf->jb_channels[i]);
	}
}
...
/* Todo esto hay que moverlo hacia abajo para que el set_owner no falle */
#if 0
  ...
  case JOINBUF_TYPE_CREATE:
	...
	set_owner(jbuf, JOINBUF_TYPE_CREATE, NULL);
    break;
