 ...
 *
 * $Id: m_burst.c,v 1.1.1.1 2006/12/19 12:55:33 zipbreake Exp $
 */
...
CMD_FUNC(ms_burst)
{
  ...
  if (!chptr->creationtime || chptr->creationtime > timestamp) {
    ...
    modebuf_rhmode(mbuf, MODE_DEL | chptr->rhmode.mode);
    chptr->mode.mode &= ~(MODE_ADD | MODE_DEL | MODE_PRIVATE | MODE_SECRET |
			  MODE_MODERATED | MODE_TOPICLIMIT | MODE_INVITEONLY |
			  MODE_NOPRIVMSGS | MODE_REGONLY);
    chptr->rhmode.mode &= ~(MODE_ADD | MODE_DEL | RHMODE_BADWORDS | RHMODE_NOCOLOR |
              RHMODE_RGSTRONLY | RHMODE_AUTOOP | RHMODE_SUSPEND | RHMODE_REGMOD |
              RHMODE_ONLYIRCOP | RHMODE_CHARLAS | RHMODE_NOCTCP | RHMODE_NONICKCHANGE);
    ...
    for (member = chptr->members; member; member = member->next_member) {
      if (member->status & CHFL_BURST_JOINED) { /* joined during burst */
		  ...
		  if (member->status & CHFL_OWNER) {
			  modebuf_mode_client(mbuf, MODE_ADD | CHFL_OWNER, member->user);
		  }
      }
		  ...
		  member->status = ((member->status & ~(CHFL_CHANOP | CHFL_VOICE | CHFL_HALFOP)) | CHFL_DEOPPED);
      }
    }
