 ...
 *
 * $Id: send.c,v 1.1.1.1 2006/12/19 12:55:26 zipbreake Exp $
 */
#include "config.h"
...
void agrega_sendq(struct Client *cptr)
{
	client_add_sendq(cli_connect(cptr), &send_queues);
}

int usa_tokens(struct Client *cptr)
{
	return !!(IsServer(cptr) && (cli_negociacion(cptr) & NEGOCIACION_TOKEN_OUT));
}
...
extern void msgq_add(struct Client *cptr, struct MsgQ *mq, struct MsgBuf *mb, int prio);
...
void send_buffer(struct Client* to, struct MsgBuf* buf, int prio)
{
  ...
  msgq_add(to, &(cli_sendQ(to)), buf, prio);
...
static int match_it(struct Client *from, struct Client *one, const char *mask, int what)
{
  ...
  {
    case MATCH_HOST:
      return (match(mask, cli_user(one)->realhost) == 0 ||
        (HasHiddenHost(one) && match(mask, cli_user(one)->virtualhost) == 0));
    case MATCH_SERVER:
    default:
		   ...
		   struct Client *to, const char *pattern, ...)
{
  ...
  mb = msgq_make(to, "%:#C %s %v", from, usa_tokens(to) || IsMe(to) ? tok : cmd,
		 &vd);
...
/** snoticeto_one()
 ** 
 ** Enviamos un 'server notice' a un cliente
 ** desde RH_PSEUDO_SERVERNAME.
 **
 ** mount@redhispana.org - 25/04/04
 **/

void snoticeto_one(struct Client *to, const char *pattern, ...)
	
{
  ...
  assert(cli_name(to) != 0);
  
  vd.vd_format = pattern; /* set up the struct VarData for %v */
  va_start(vd.vd_args, pattern);

  mb = msgq_make(to, ":%s NOTICE %s :%v", feature_str(FEAT_RH_PSEUDO_SERVERNAME), cli_name(to), &vd);
...
/*
 * Enviar un comando con un bot virtual
 *
 */
void sendbotcmd(char *botname, const char *cmd, const char *tok, struct Client *to,
				const char *pattern, ...)
{
	struct VarData vd;
	struct MsgBuf *mb;
 ...
 */
static void vsendcmdto_one(struct Client *from1, struct Client *from, const char *cmd,
						   const char *tok, struct Client *to, struct VarData vd)
{
	struct MsgBuf *mb;

	mb = msgq_make(from1, "%C %s %v", from, usa_tokens(to) ? tok : cmd, &vd);
	send_buffer(to, mb, 0);
	msgq_clean(mb);
}
...
//  va_end(vd.vd_args);
  ...
  for (lp = cli_serv(&me)->down; lp; lp = lp->next) {
    if (one && lp->value.cptr == cli_from(one))
      continue;
	vsendcmdto_one(&me, from, cmd, tok, lp->value.cptr, vd);
...
void sendcmdto_channel_butserv_butone_botmode(char *botname, const char *cmd,
				      const char *tok, struct Channel *to,
				      struct Client *one, const char *pattern,
				      ...)
{
...
void sendcmdto_channel_butservs_butone_botmode(char *botname, const char *cmd,
				      const char *tok, struct Channel *to,
				      struct Client *one, const char *pattern,
				      ...)
{
...
//      send_buffer(member->user, serv_mb, 0);
		vsendcmdto_one(&me, from, usa_tokens(cli_from(member->user)) ? tok : cmd, tok, member->user, vl);
	}
  }
			     ...
			     const char *pattern, ...)
{
  ...
  /* Build buffer to send to servers */
  va_start(vd.vd_args, pattern);
  mb = msgq_make(&me, "%C %s :%v", from, cmd, &vd);
  va_end(vd.vd_args);

  /* Creamos el buffer para mandar a servidores con soporte de tokens */
  va_start(vd.vd_args, pattern);
  mb_token = msgq_make(&me, "%C %s :%v", from, tok, &vd);
  va_end(vd.vd_args);
  ...
  for (lp = cli_serv(&me)->down; lp; lp = lp->next) {
    if (one && lp->value.cptr == cli_from(one))
      continue;
    send_buffer(lp->value.cptr, usa_tokens(lp->value.cptr) ? mb_token: mb, 1);
  }
...
//  struct MsgBuf *serv_mb;
  ...
  /* Build buffer to send to users */
  va_start(vl, pattern);
  token = (char *)va_arg(vl, char *);
  msg = (char *)va_arg(vl, char *);
  va_end(vl);
/*  va_start(vd.vd_args, pattern);
...
//      send_buffer(cptr, user_mb, 0);
		sendcmdto_one(from, cmd, tok, cptr, "%C :*** Mensaje Global -> (%s): %s", cptr, token, msg);
	}
    else {
		vsendcmdto_one(&me, from, cmd, tok, cptr, vd);
